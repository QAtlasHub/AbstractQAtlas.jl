...
@testset "registry + traits" begin
    rels = all_relations()
    @test length(rels) == 101         # universal-only: model-specific (spin glass, Drude mobility, single-band Hall) moved to QAtlas
    ...
    @test length(all_relations(; domain=:entanglement)) == 23
