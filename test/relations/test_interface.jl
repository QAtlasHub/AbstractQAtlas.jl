...
@testset "registry + traits" begin
    rels = all_relations()
    @test length(rels) == 143        # +5 infinite-randomness (see :scaling below); +ActivatedDynamicalScaling, +TypicalBelowAverage, +AnnealedFreeEnergyBound; universal-only: model-specific (spin glass, Drude mobility, single-band Hall) moved to QAtlas; +LoschmidtRate; +8 universal bounds; +ElectricCurrentResponse
    @test allunique(typeof.(rels))
    @test length(all_relations(; domain=:scaling)) == 11   # +ActivatedDynamicalScaling; +TypicalCorrelationLength, GriffithsExponentDivergence, GriffithsSusceptibility, GriffithsSpecificHeat, ActivatedMomentGrowth
